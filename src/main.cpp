...
#define uS_TO_S_FACTOR \
  1000000uL /* Conversion factor for micro seconds to seconds */
//#define uS_TO_ms_FACTOR 1000000uL  /* Conversion factor for micro seconds to
// seconds */ #define TIME_TO_SLEEP  5uL //5 seconds #define TIME_TO_SLEEP
// 1800uL
////1/2 Stunde
...
Baro baro;
beeper Beeper(BEEP_VELOCITY_DEFAULT_SINKING_THRESHOLD,
              BEEP_VELOCITY_DEFAULT_CLIMBING_THRESHOLD,
              BEEP_VELOCITY_DEFAULT_NEAR_CLIMBING_SENSITIVITY,
              BEEP_DEFAULT_VOLUME);
int freq = 2000;
int channel = 0;
int resolution = 8;
...
void Fanet2FlarmData(FanetLora::trackingData *FanetData,
...
void readFuelSensor(uint32_t tAct) {
  ...
  if (timeOver(tAct, tRead, 100)) {  // every 100ms
    ...
    analogRead(
        PinFuelSensor);  // First measurement has the biggest difference on my
                         // board, this line just skips the first measurement
  ...
  if (timeOver(tAct, tSend, FUELSENDINTERVALL)) {
    ...
    char sOut[MAXSTRING];
    int pos = 0;
    pos += snprintf(&sOut[pos], MAXSTRING - pos, "$FUEL,%.03f,",
                    status.fuelSensor);
...
void oledPowerOn() {
  ...
  // initialize OLED
  if (!display.begin(SSD1306_SWITCHCAPVCC, OLED_SLAVE_ADDRESS, false,
  ...
  // added the possibility to invert BW .. whould be nice to put it in the
  // settings TODO
...
void printChipInfo(void) {
  ...
  log_i("This is ESP32 chip with %d CPU cores, WiFi%s%s, ", chip_info.cores,
...
bool printLocalTime() {
  ...
  if (now.tm_year >= 117) {
    ...
    log_i("%04d %02d %02d %02d:%02d:%02d", now.tm_year + 1900, now.tm_mon + 1,
...
== false)) { longPressActive = true; if (setting.vario.volume == LOWVOLUME){
    ...
    //Button released
                if (!setting.bHasExtPowerSw){ //no muting when in kobo (for the
moment) if (buttonActive == true) { if (longPressActive == false) {
  ...
  if (timeOver(tAct, tSend, FLARM_UPDATE_RATE)) {
    ...
    if (status.GPS_Fix) {
      ...
      for (int i = 0; i < MAXNEIGHBOURS; i++) {
        // if ((fanet.neighbours[i].devId) && (fanet.neighbours[i].type ==
        // 0x11)){ //we have a ID an we are flying !!
        if (fanet.neighbours[i].devId) {  // we have a ID an we are flying !!
          int pos;
          char sOut[MAXSTRING];
          if (fanet.neighbours[i].addressType == 0xff80) {
            pos = fanet.writeFanetData(sOut, MAXSTRING, &fanet.neighbours[i]);
          } else {
            ...
            pos = flarm.writeFlarmData(sOut, MAXSTRING, &myFlarmData,
                                       &PilotFlarmData);
          }
          sendData2Client(sOut, pos);
          countNeighbours++;
...
void checkFlyingState(uint32_t tAct) {
  ...
  if (status.flying) {
    ...
    // on ground
    if ((status.GPS_Fix) && (status.GPS_NumSat >= 4) &&
        (status.GPS_speed > MIN_FLIGHT_SPEED)) {  // minimum 4 satelites to get
                                                  // accurade position and speed
...
void drawAircraftType(int16_t x, int16_t y, uint8_t AircraftType) {
  switch (AircraftType) {
    ...
    case FanetLora::leg_para_glider:
      display.drawXBitmap(x, y, Paraglider16_bits, 16, 16,
                          WHITE);  // GxEPD_BLACK);
      break;
    case FanetLora::hangglider:
    case FanetLora::leg_hang_glider:
      display.drawXBitmap(x, y, Hangglider16_bits, 16, 16,
                          WHITE);  // GxEPD_BLACK);
      break;
...
void printGSData(uint32_t tAct) {
  ...
  switch (fanet.neighbours[index].aircraftType) {
    ...
    case FanetLora::leg_aircraft_reciprocating_engine:
      display.drawXBitmap(88, 12, Airplane40_bits, Airplane40_width,
                          Airplane40_height, WHITE);
      break;
    case FanetLora::aircraft_t::helicopter:
    case FanetLora::leg_helicopter_rotorcraft:
      display.drawXBitmap(88, 10, Helicopter40_bits, Helicopter40_width,
                          Helicopter40_height, WHITE);
      break;
...
uint32_t calcSleepTime() {
  ...
  if (iAct > iSunSet) {
    iRet = (1440 - iAct + iSunRise) *
           60;  // we have to calc until midnight and then until sunrise
  ...
  if ((iSunRise >= 0) && (iSunSet >= 0)) {
    ...
    if (iSunRise < iSunSet) {
      if ((iAct >= iSunRise) && (iAct <= iSunSet)) {
        ...
        printLocalTime();
        log_i("%02d:%02d:%02d sunrise=%s sunset=%s iAct=%s", now.tm_hour,
              now.tm_min, now.tm_sec, time1, time2, time3);
        return 0;
      }
    } else {
...
void enterDeepsleep() {
  ...
  log_i("time to sleep = %02d:%02d:%02d", tSleep / 60 / 60, (tSleep / 60) % 60,
        tSleep % 60);
  esp_sleep_enable_timer_wakeup((uint64_t)tSleep *
                                uS_TO_S_FACTOR);  // set Timer for wakeup
...
void sendAWGroundStationdata(uint32_t tAct) {
  static uint32_t tSend =
      millis() - 290000;                // 10sec. delay, to get wifi working
...
void sendTraccarTrackingdata(FanetLora::trackingData *FanetData) {
  if ((!setting.traccarLiveTracking) ||
      (!setting.TraccarSrv.startsWith("http://")) ||
      (WiFi.status() != WL_CONNECTED) || (!status.bTimeOk))
    return;

  // HTTPClient http;
  // WiFiClient c;
  // HttpClient http(c);
  String server = setting.TraccarSrv.substring(strlen("http://"),
                                               setting.TraccarSrv.length());
  int iPort = 80;
...
#endif
  msg += setting.myDevId + "," + fanet.getDevId(FanetData->devId) + "," +
...
void sendData2Client(char *buffer, int iLen) {
  ...
  if (setting.outputMode == OUTPUT_UDP) {
    ...
    if ((WiFi.status() == WL_CONNECTED) ||
        (WiFi.softAPgetStationNum() >
         0)) {  // connected to wifi or a client is connected to me
      // log_i("sending udp");
  ...
  if ((setting.outputMode == OUTPUT_SERIAL) ||
      (setting.bOutputSerial)) {  // output over serial-connection
    Serial.print(buffer);
  }
...
void setupAXP192() {
  ...
  if (i2cOLED.endTransmission() == 0) {
    if (!axp.begin(i2cOLED, AXP192_SLAVE_ADDRESS)) {
      ...
      attachInterrupt(digitalPinToInterrupt(AXP_IRQ), AXP192_Interrupt_handler,
                      FALLING);
...
void setupWifi() {
  ...
  // WiFi.config(INADDR_NONE, INADDR_NONE, INADDR_NONE); // call is only a
  // workaround for bug in WiFi class
  WiFi.config(INADDR_NONE, INADDR_NONE, INADDR_NONE, INADDR_NONE, INADDR_NONE);
  // if (WiFi.config(INADDR_NONE, INADDR_NONE,
  // INADDR_NONE,INADDR_NONE,INADDR_NONE) == false){
  ...
  // if (WiFi.softAP(host_name.c_str(), setting.appw.c_str(),rand() % 12 +
  // 1,0,2)){
  ...
  if (setting.wifi.connect != WIFI_CONNECT_NONE) {
    ...
    if ((WiFi.SSID() != setting.wifi.ssid ||
         WiFi.psk() != setting.wifi.password)) {
  ...
  if ((WiFi.getMode() & WIFI_MODE_STA)) {
    if (setting.outputMode != OUTPUT_BLE) {
      WiFi.setSleep(
          false);  // disable power-save-mode !! will increase ping-time
    }
  }
  ...
  TCPIP_ADAPTER_DOMAIN_NAME_SERVER, &opt_val, sizeof(dhcps_offer_t));  // don't
  supply a dns server via dhcps opt_val = 0; // no Router
  tcpip_adapter_dhcps_option(TCPIP_ADAPTER_OP_SET,
  TCPIP_ADAPTER_ROUTER_SOLICITATION_ADDRESS, &opt_val, sizeof(dhcps_offer_t));
  // don't supply a gateway (router) via dhcps option 3
...
void startOLED() {
  ...
  for 128x32 log_e("SSD1306 allocation failed"); for(;;); // Don't proceed, loop
  forever
  }
  */
  oledPowerOn();
  ...
  display.display();
  delay(1000);
  display.drawXBitmap(69, 2, AirCom_Logo_bits, AirCom_Logo_width,
                      AirCom_Logo_height, WHITE);
  display.setTextSize(1);
...
void printSettings() {
  ...
  log_i("Aircraft=%s",
        fanet.getAircraftType((FanetLora::aircraft_t)setting.AircraftType)
            .c_str());
  ...
  log_i("VarioNearClimbingSensitivity=%0.2f",
        setting.vario.nearClimbingSensitivity);
...
void startBluetooth(void){
  if (setting.outputMode == OUTPUT_BLE){
    ...
    //log_i("currHeap:%d,minHeap:%d", xPortGetFreeHeapSize(),
xPortGetMinimumEverFreeHeapSize()); }else if (setting.outputMode ==
OUTPUT_BLUETOOTH){
    ...
    log_i("currHeap:%d,minHeap:%d", xPortGetFreeHeapSize(),
xPortGetMinimumEverFreeHeapSize()); status.bluetoothStat = 1;
  }
}
*/
...
void testLegacy() {
  log_i("test legacy");
  // uint8_t rx_frame[26] =
  // {0x7A,0x32,0x4B,0x10,0x82,0xE5,0x59,0x4E,0x30,0x69,0x70,0xCB,0x4F,0xFC,0x5C,0x83,0xD1,0xB0,0xB7,0xF0,0x82,0x38,0xD4,0xC0,0xCD,0x34};
  uint8_t rx_frame[26] = {0x21, 0x30, 0x4B, 0x10, 0x99, 0x63, 0xA9, 0x3B, 0xAA,
                          0x3A, 0xAE, 0x2A, 0x12, 0xEC, 0xE9, 0xC5, 0xE4, 0x6E,
                          0x54, 0xA7, 0x2C, 0x0E, 0x70, 0xD3, 0x81, 0x7E};
  ...
  for (int i = 0; i < 5; i++) {
    ...
    if (ret == 0) {
      float dist = distance(myAircraft.latitude, myAircraft.longitude,
                            air.latitude, air.longitude, 'K');
      // if ((dist <= 100.0) && (air.addr != 0) && (air.aircraft_type != 0)){
      if ((air.addr != 0) && (air.aircraft_type != 0)) {
        len = sprintf(Buffer,
                      "adr=%06X;adrType=%d;airType=%d,lat/"
                      "lon=%.06f,%.06f,alt=%.01f,speed=%.01f,course=%.01f,"
                      "climb=%.01f,dist=%.01f\n",
                      air.addr, air.addr_type, air.aircraft_type, air.latitude,
                      air.longitude, air.altitude, air.speed, air.course,
                      air.vs, dist);
        log_i("%s", Buffer);
...
void setup() {
  ...
  log_i("current free heap: %d, minimum ever free heap: %d",
        xPortGetFreeHeapSize(), xPortGetMinimumEverFreeHeapSize());

  // esp_sleep_wakeup_cause_t reason = print_wakeup_reason(); //print reason for
  // wakeup
  esp_sleep_wakeup_cause_t reason2 =
  ...
  log_i("SPIFFS total=%d used=%d free=%d", SPIFFS.totalBytes(),
  ...
  setting.displayType = NO_DISPLAY;
#endif
  status.displayType = setting.displayType;  // we have to copy the display-type
                                             // in case, setting is changing
  ...
  */
...
#ifdef GSMODULE
  if (setting.Mode == MODE_GROUND_STATION) {
    if ((reason2 == ESP_SLEEP_WAKEUP_TIMER) &&
        (setting.gs.PowerSave == GS_POWER_SAFE)) {
      printLocalTime();
      if (isDayTime() == 0) {
        log_i("not day --> enter deep-sleep");
        uint32_t tSleep = calcSleepTime();
        // log_i("time to sleep = %d",tSleep);
        log_i("time to sleep = %d --> %02d:%02d:%02d", tSleep, tSleep / 60 / 60,
              (tSleep / 60) % 60, (tSleep) % 60);
        esp_sleep_enable_timer_wakeup((uint64_t)tSleep *
                                      uS_TO_S_FACTOR);  // set Timer for wakeup
        esp_wifi_stop();
        esp_bluedroid_disable();
...
#endif
  if ((setting.wifi.ssid.length() <= 0) ||
      (setting.wifi.password.length() <= 0)) {
    setting.wifi.connect =
        WIFI_CONNECT_NONE;  // if no pw or ssid given --> don't connecto to wifi
  }

  // pinMode(BUTTON2, INPUT_PULLUP);

  printSettings();
  analogReadResolution(10);  // Default of 12 is not very linear. Recommended to
                             // use 10 or 11 depending on needed resolution.
  status.bHasAXP192 = false;
  ...
  switch (setting.boardType) {
    ...
    case BOARD_T_BEAM_V07:
      ...
      // moving SCL SDA to different GPIO to avoid conflicts with mounted SD
      // card on Lilygo T3 v2.1.1.6
      // TTGO V0.7 has no pin 4 -> switched back
      PinBaroSDA = 13;  // 13 3;
      PinBaroSCL = 14;  // 14 23;
      // set gpio 4 as INPUT
      ...
      // vIn = (R1+R2)/R2 * VOut
      adcVoltageMultiplier =
          2.0f * 3.5f;  // maybe also 3.4 or 3.45 .. TODO test
    ...
    case BOARD_TTGO_T3_V1_6:
      ...
      // voltage-divier 100kOhm and 100kOhm
      // vIn = (R1+R2)/R2 * VOut
      adcVoltageMultiplier = 2.0f * 3.76f; // not sure if it is ok ?? don't have
    this kind of board pinMode(PinADCVoltage, INPUT); break;
    */
    ...
    case BOARD_HELTEC_WIRELESS_STICK_LITE:
      ...
      // 1S LiPo
      pinMode(21,
              OUTPUT);  // we have to set pin 21 to measure voltage of Battery
      digitalWrite(21, LOW);  // set output to Low, so we can measure the
                              // voltage
    ...
    case BOARD_TTGO_TSIM_7000:
      ...
      if (setting.displayType > 1) {
        PinOneWire =
            -1;  // no one-wire if display is eink, cause we need that pin
      } else {
        PinOneWire = 25;
      }
      ...
      adcVoltageMultiplier =
          2.0f *
          3.76f;  // not sure if it is ok ?? don't have this kind of board
      pinMode(PinADCVoltage, INPUT);
    ...
    case BOARD_TTGO_TCALL_800:
      ...
      adcVoltageMultiplier =
          2.0f *
          3.50f;  // not sure if it is ok ?? don't have this kind of board
      pinMode(PinADCVoltage, INPUT);
...
#ifdef GSMODULE
  if (setting.Mode == MODE_GROUND_STATION) {
    // if ((reason2 == ESP_SLEEP_WAKEUP_TIMER) && (setting.gs.PowerSave ==
    // GS_POWER_BATT_LIFE)){
    if ((setting.gs.PowerSave == GS_POWER_BATT_LIFE) ||
      ...
      if ((status.vBatt >= BATTPINOK) &&
          (status.BattPerc <
           (setting.minBattPercent + setting.restartBattPercent)) &&
          (status.BattPerc < 80)) {
        // go again to sleep
        printLocalTime();
        log_i("batt-voltage to less (%d%%) --> going to sleep again for 60min.",
              status.BattPerc);
        esp_sleep_enable_timer_wakeup((uint64_t)BATTSLEEPTIME *
                                      uS_TO_S_FACTOR);  // set Timer for wakeup
        esp_wifi_stop();
        esp_bluedroid_disable();
        esp_bluedroid_deinit();
...
// adding logger task
#ifdef LOGGER
    xTaskCreatePinnedToCore(taskLogger, "taskLogger", 6500, NULL, 4,
                            &xHandleLogger,
                            ARDUINO_RUNNING_CORE1);  // background Logger
#endif

    xTaskCreatePinnedToCore(taskBaro, "taskBaro", 6500, NULL, 9, &xHandleBaro,
                            ARDUINO_RUNNING_CORE1);  // high priority task
  }
#endif
  // log_i("currHeap:%d,minHeap:%d", xPortGetFreeHeapSize(),
  // xPortGetMinimumEverFreeHeapSize());
  xTaskCreatePinnedToCore(taskStandard, "taskStandard", 6500, NULL, 10,
                          &xHandleStandard,
                          ARDUINO_RUNNING_CORE1);  // standard task
  // log_i("currHeap:%d,minHeap:%d", xPortGetFreeHeapSize(),
  // xPortGetMinimumEverFreeHeapSize());

#ifdef EINK
  xTaskCreatePinnedToCore(taskEInk, "taskEInk", 6500, NULL, 8, &xHandleEInk,
                          ARDUINO_RUNNING_CORE1);  // background EInk
#endif
  xTaskCreatePinnedToCore(taskBackGround, "taskBackGround", 6500, NULL, 5,
                          &xHandleBackground,
                          ARDUINO_RUNNING_CORE1);  // background task
  xTaskCreatePinnedToCore(taskBluetooth, "taskBluetooth", 4096, NULL, 7,
                          &xHandleBluetooth, ARDUINO_RUNNING_CORE1);

#ifdef GSMODULE
  if (setting.Mode == MODE_GROUND_STATION) {
    // start weather-task
    xTaskCreatePinnedToCore(taskWeather, "taskWeather", 13000, NULL, 8,
                            &xHandleWeather, ARDUINO_RUNNING_CORE1);
  }
#endif
#ifdef GSM_MODULE
  // start Gsm-task
  xTaskCreatePinnedToCore(taskGsm, "taskGsm", 4096, NULL, 3, &xHandleGsm,
                          ARDUINO_RUNNING_CORE1);
#endif
}
...
bool connectGPRS() {
  log_i("Connecting to internet apn=%s,user=%s,pw=%s", setting.gsm.apn.c_str(),
        setting.gsm.user.c_str(), setting.gsm.pwd.c_str());
  return modem.gprsConnect(setting.gsm.apn.c_str(), setting.gsm.user.c_str(),
                           setting.gsm.pwd.c_str());
}
...
void PowerOffModem() {
  ...
  modem.sleepEnable(false);  // required in case sleep was activated and will
                             // apply after reboot
  for (int i = 0; i < 10; i++) {
...
void setupSim7000Gps() {
  ...
  // modem.sendAT(GF("+CGNSCFG=1")); //Turn on GNSS NMEA data out put to USB’s
  // NMEA port modem.sendAT(GF("+CGNSCFG=2")); //Turn on GNSS NMEA data out put
  // to UART3 port modem.waitResponse();
...
void taskGsm(void *pvParameters) {
  ...
  GsmSerial.begin(115200, SERIAL_8N1, PinGsmRx, PinGsmTx,
                  false);  // baud, config, rx, tx, invert
  ...
  static uint32_t tCheckConn =
      millis() - GSM_CHECK_TIME_CON;  // check every 60sec.
  ...
  while (1) {
    ...
    if (command.getGpsPos == 1) {
      ...
      if (modem.getGPS(&lat, &lon, &speed, &alt, &vsat, &usat, &accuracy)) {
        log_i(
            "GPS-Position: lat:%.6f lon:%.6f alt:%.2f vsat:%d usat:%d "
            "accuracy:%.2f",
            lat, lon, alt, vsat, usat, accuracy);
...
void taskWeather(void *pvParameters) {
  ...
  static uint32_t tSendData =
      millis() + 10000;  // first sending is in 10 seconds
  bool bDataOk = false;
  ...
  if (weather.begin(&i2cWeather, setting.gs.alt, PinOneWire, PinWindDir,
                    PinWindSpeed, PinRainGauge)) {
  ...
  while (1) {
    ...
    if (status.vario.bHasBME) {
      ...
      if (weather.getValues(&wData)) {
        ...
        for (int i = 0; i < 2; i++) {
          ...
          avg[i].sinWinddir = calcExpAvgf(avg[i].sinWinddir,
                                          sin(wData.WindDir * DEG2RAD), fAvg);
          avg[i].cosWinddir = calcExpAvgf(avg[i].cosWinddir,
                                          cos(wData.WindDir * DEG2RAD), fAvg);
          // log_i("%d,cos=%f,sin=%f",i,avg[i].sinWinddir,avg[i].cosWinddir);
          avg[i].Winddir = atan2(avg[i].sinWinddir, avg[i].cosWinddir) *
                           RAD2DEG;  // get deg back vom sin and cos-value
          ...
          avg[i].WindSpeed =
              calcExpAvgf(avg[i].WindSpeed, wData.WindSpeed, fAvg);
          if (wData.WindSpeed > avg[i].WindGust)
            avg[i].WindGust = wData.WindSpeed;
        ...
        status.weather.WindSpeed =
            avg[0].WindSpeed;  // we use the Fanet-Weather-Speed
        status.weather.WindGust =
            avg[0].WindGust;  // we use the Fanet-Weather-Gust
      }
      if (timeOver(tAct, tUploadData, setting.wd.WUUploadIntervall)) {
        ...
        if ((status.bInternetConnected) && (status.bTimeOk)) {
          ...
          if (setting.WindyUpload.enable) {
            ...
            wi.sendData(setting.WindyUpload.ID, setting.WindyUpload.KEY,
                        &wiData);
          }
        }
      ...
      if (timeOver(tAct, tSendData, setting.wd.FanetUploadInterval)) {
        ...
        if (timeStatus() == timeSet) {
          ...
          pWd = &msg_buf[0];
          sprintf(buff, "%04d-%02d-%02d %02d:%02d:%02d", year(), month(), day(),
                  hour(), minute(), second());
          doc["DT"] = buff;
    ...
    if (status.bWUBroadCast) {
      ...
      if (timeOver(tAct, tUploadData,
                   setting.wd.WUUploadIntervall)) {  // get Data from WU
        tUploadData = tAct;
...
#endif
          bDataOk =
              wu.getData(setting.WUUpload.ID, setting.WUUpload.KEY, &wuData);
          if (bDataOk) {
            ...
            fanetWeatherData.bStateOfCharge = true;
            log_i(
                "winddir=%.1f speed=%.1f gust=%.1f temp=%.1f hum=%.1f "
                "press=%.1f",
                fanetWeatherData.wHeading, fanetWeatherData.wSpeed,
                fanetWeatherData.wGust, fanetWeatherData.temp,
                fanetWeatherData.Humidity, fanetWeatherData.Baro);
          } else {
...
void taskBaro(void *pvParameters) {
  ...
  if (baroSensor > 0) {
    ...
    Beeper.setThresholds(setting.vario.sinkingThreshold,
                         setting.vario.climbingThreshold,
                         setting.vario.nearClimbingSensitivity);
    ...
    while (1) {
      ...
      */
      ...
      if (setting.vario.useMPU) {
        baro.getMPUValues(&status.vario.accel[0], &status.vario.gyro[0],
                          &status.vario.acc_Z);
      }
      if (baro.isNewVAlues()) {
        baro.getValues(&status.pressure, &status.varioAlt, &status.ClimbRate,
                       &status.varioTemp);
...
void taskBluetooth(void *pvParameters) {
  ...
  if ((setting.outputMode == OUTPUT_BLE) ||
      (setting.outputMode == OUTPUT_BLUETOOTH)) {
  } else {
    ...
    while (1) {
      ...
      if (xPortGetFreeHeapSize() > BLE_LOW_HEAP) {
        ...
        */
        ...
        log_d(
            " BLE congested - Waiting - Current free heap: %d, minimum ever "
            "free heap: %d",
            xPortGetFreeHeapSize(), xPortGetMinimumEverFreeHeapSize());
        ble_mutex = true;
...
float readBattvoltage() {
  ...
  analogRead(
      PinADCVoltage);  // First measurement has the biggest difference on my
                       // board, this line just skips the first measurement
  for (int i = 0; i < NO_OF_SAMPLES; i++) {
...
void printBattVoltage(uint32_t tAct) {
  ...
  if ((tAct - tBatt) >= 5000) {
    ...
    if (status.vBatt >= battFull){
      log_i("Battery Full: %d mV Temp:%s
    C",status.vBatt,String(status.varioTemp,1));
  ...
  if (timeOver(tAct, tRefresh, 3000)) {
    ...
    switch (screen) {
      ...
      case 2:  // speed and gust
        s = String(round(status.weather.WindSpeed), 0) + "|" +
            String(round(status.weather.WindGust), 0);  // + "kh";
        break;
      case 3:  // temp
...
void printScanning(uint32_t tAct) {
  ...
  switch (icon) {
    case 0:
      display.drawXBitmap(1, 10, Antenna_bits, Antenna_width, Antenna_height,
                          WHITE);
      break;
    case 1:
      display.drawXBitmap(1, 10, Antenna_bits, Antenna_width, Antenna_height,
    ...
    case 11:
      ...
      display.drawXBitmap(42, 10, WFTX_bits, WFTX_width, WFTX_height, WHITE);
      display.drawXBitmap(66, 34, WFRX_bits, WFRX_width, WFRX_height, WHITE);
      display.drawXBitmap(88, 10, Airplane40_bits, Airplane40_width,
                          Airplane40_height, WHITE);
      break;
    case 12:
      display.drawXBitmap(1, 10, Antenna_bits, Antenna_width, Antenna_height,
...
void DrawRadarPilot(uint8_t neighborIndex) {
  ...
  if (fanet.neighbours[neighborIndex].name.length() > 0) {
    display.print(fanet.neighbours[neighborIndex].name.substring(
        0, 10));  // max. 10 signs
  } else {
    display.print(fanet.getDevId(fanet.neighbours[neighborIndex].devId));
  }
  pilotDistance = distance(fanet._myData.lat, fanet._myData.lon,
                           fanet.neighbours[neighborIndex].lat,
                           fanet.neighbours[neighborIndex].lon, 'K') *
                  1000;
  bearing = CalcBearingA(fanet._myData.lat, fanet._myData.lon,
                         fanet.neighbours[neighborIndex].lat,
                         fanet.neighbours[neighborIndex].lon);
  ...
  display.printf("%sm",
                 setStringSize(String(fanet.neighbours[neighborIndex].altitude -
                                          fanet._myData.altitude,
                                      0),
                               5)
                     .c_str());
  display.setCursor(75, 52);  // display climbing
  display.printf(
      "%sms", setStringSize(String(fanet.neighbours[neighborIndex].climb, 1), 5)
                  .c_str());
}
...
void DrawRadarScreen(uint32_t tAct, uint8_t mode) {
  ...
  DrawAngleLine(RADAR_SCREEN_CENTER_X, RADAR_SCREEN_CENTER_Y, 30,
                fanet._myData.heading * -1);
  DrawAngleLine(RADAR_SCREEN_CENTER_X, RADAR_SCREEN_CENTER_Y, 6,
                (fanet._myData.heading + 90) * -1);
...
void setWifi(bool on) {
  if ((on) && (status.wifiStat == 0)) {
    ...
    // workaround for bug in WiFi class
    WiFi.config(
        INADDR_NONE, INADDR_NONE, INADDR_NONE, INADDR_NONE,
        INADDR_NONE);  // call is only a workaround for bug in WiFi class
    // WiFi.config(local_IP, INADDR_NONE, subnet,INADDR_NONE,INADDR_NONE); //
...
void checkSystemCmd(char *ch_str) {
  ...
  if (line.indexOf("#SYC FUEL_SENSOR?") >= 0) {
    // log_i("sending 2 client");
    add2OutputString(
        "#SYC FUEL_SENSOR=" + String((uint8_t)setting.bHasFuelSensor) + "\r\n");
  }
...
void readGPS() {
  ...
  if (sNmeaIn.length() > 0) {  // String received by Bluetooth
    if (!status.bHasGPS) {
      ...
      if ((setting.outputGPS) && (!WebUpdateRunning))
        sendData2Client(cstr, sNmeaIn.length());  // sendData2Client(sNmeaIn);
  ...
  if ((setting.Mode == MODE_AIR_MODULE) ||
      ((abs(setting.gs.lat) <= 0.1) && (abs(setting.gs.lon) <= 0.1))) {
    ...
    while (NMeaSerial.available()) {
      ...
      if (lineBuffer[recBufferIndex] == '\n') {
        ...
        lineBuffer[recBufferIndex] = 0;  // zero-termination
        if ((setting.outputGPS) && (!WebUpdateRunning))
          sendData2Client(lineBuffer, recBufferIndex);
        recBufferIndex = 0;
        tGpsOk = millis();
        if (!status.bHasGPS) {
          fanet.setGPS(true);
...
void sendLXPW(uint32_t tAct) {
  ...
  if ((tAct - tOld) >= 250) {
    ...
    if (status.vario.bHasVario) {
      pos += snprintf(
          &sOut[pos], MAXSTRING - pos, "%.01f,%.02f", status.varioAlt,
          status.ClimbRate);  // altitude in meters, relative to QNH 1013.25
    } else {
      pos += snprintf(&sOut[pos], MAXSTRING - pos, ",");
    }
    pos += snprintf(&sOut[pos], MAXSTRING - pos, ",,,,,,");
    ...
    s += ",";
    if (status.vario.bHasVario){
      s += String(status.varioAlt,1) + "," + String(status.ClimbRate,2); //
    altitude in meters, relative to QNH 1013.25
    }
    s+=  + ",,,,,,";
    if (status.GPS_Fix){
...
void sendLK8EX(uint32_t tAct) {
  ...
  if ((tAct - tOld) >= 100) {
    ...
    if (status.vario.bHasVario) {
      pos += snprintf(&sOut[pos], MAXSTRING - pos, "%d,",
                      (uint32_t)status.pressure);
      if (status.GPS_Fix) {
        pos += snprintf(&sOut[pos], MAXSTRING - pos, "%.02f,", status.GPS_alt);
      } else {
        pos += snprintf(&sOut[pos], MAXSTRING - pos, "%.02f,", status.varioAlt);
      }
      pos += snprintf(&sOut[pos], MAXSTRING - pos, "%d,%.01f,",
                      (int32_t)(status.ClimbRate * 100.0), status.varioTemp);
    } else {
      pos += snprintf(&sOut[pos], MAXSTRING - pos, "999999,%.02f,%d,99,",
                      status.GPS_alt, (int32_t)(status.ClimbRate * 100.0));
    }
    pos += snprintf(&sOut[pos], MAXSTRING - pos, "%.02f,",
                    (float)status.vBatt / 1000.);
    pos = flarm.addChecksum(sOut, MAXSTRING);
    sendData2Client(sOut, pos);
    /*
    String s = "$LK8EX1,";
    ...
    if (status.vario.bHasVario){
      s += String(status.pressure,2) + ","; //raw pressure in hPascal: hPA*100
    (example for 1013.25 becomes  101325) if (status.GPS_Fix){ s +=
    String(status.GPS_alt,2) + ","; // altitude in meters, relative to QNH
    1013.25 }else{ s += String(status.varioAlt,2) + ","; // altitude in meters,
    relative to QNH 1013.25
      }
      s += String((int32_t)(status.ClimbRate * 100.0)) + ","; //climbrate in
    cm/s s += String(status.varioTemp,1) + ","; //temperature }else{ s +=
    "999999,"; //raw pressure in hPascal: hPA*100 (example for 1013.25 becomes
    101325) s += String(status.GPS_alt,2) + ","; // altitude in meters, relative
    to QNH 1013.25 s += String((int32_t)(status.ClimbRate * 100.0)) + ",";
    //climbrate in cm/s
      //s += String(status.) + ",";
      s += "99,"; //temperature
    }
...
void taskStandard(void *pvParameters) {
  ...
  if (setting.boardType == BOARD_T_BEAM_SX1262){
    ...
    if (status.bHasAXP192){
      ...
      // moving gpio for GPS back to 34 to avoid conflicts with SD pins on
  LilyGo T3 v2.1.1.6 PinGPSRX = 34;//12; PinGPSTX = 39;//15;
    }
  }
  */
  if (PinGPSRX >= 0) {
    ...
    // clear serial buffer
    while (NMeaSerial.available()) NMeaSerial.read();
  }
  ...
  if (setting.Mode == MODE_GROUND_STATION) {
    ...
    fanet.setMyTrackingData(&MyFanetData, setting.gs.geoidAlt,
                            0);  // set Data on fanet
  }
#endif
  ...
  // bool begin(int8_t sck, int8_t miso, int8_t mosi, int8_t ss,int reset, int
  // dio0,long frequency,uint8_t outputPower);
  ...
  fanet.begin(PinLora_SCK, PinLora_MISO, PinLora_MOSI, PinLora_SS, PinLoraRst,
              PinLoraDI0, PinLoraGPIO, frequency, setting.LoraPower, radioChip);
  fanet.setPilotname(setting.PilotName);
  fanet.setAircraftType(FanetLora::aircraft_t(setting.AircraftType));
  ...
  host_name = APPNAME "-" +
              setting.myDevId;  // String((ESP32_getChipId() & 0xFFFFFF), HEX);
#ifdef AIRMODULE
  if (setting.Mode == MODE_AIR_MODULE) {
    flarm.begin();
  }
  ...
  while (1) {
    ...
    if (timeOver(tAct, tSend, 1000)) {
      tSend = tSend;
      sendData2Client(
          "$GPGGA,161640.00,4804.36384,N,01444.09266,E,1,04,11.84,320.6,M,43.5,"
          "M,,*69\n");
      sendData2Client(
          "$GPRMC,161640.00,A,4804.36384,N,01444.09266,E,0.867,,090121,,,A*"
          "7A\n");
      sendData2Client("#FNF 8,C3CC,1,0,1,B,F33344467E0A0A12397CB8\n");
      sendData2Client(
          "#FNF 8,C3CC,1,0,2,13,416E64726561732048696E74657265636B6572\n");
    }
#endif
    ...
    if (setting.OGNLiveTracking.mode > 0) {
      if (status.vario.bHasVario) {
        ogn.setStatusData(status.pressure, status.varioTemp, NAN,
                          (float)status.vBatt / 1000., status.BattPerc);
      } else if ((status.vario.bHasBME) || (status.bWUBroadCast)) {
        ogn.setStatusData(status.weather.Pressure, status.weather.temp,
                          status.weather.Humidity, (float)status.vBatt / 1000.,
                          status.BattPerc);
      } else {
    ...
    if (sendFanetData == 1) {
      ...
      log_i("sending msgtype 3 to %s %s", fanet.getDevId(fanetReceiver).c_str(),
            fanetString.c_str());
    ...
    fanet.getRxTxCount(&status.fanetRx, &status.fanetTx, &status.legRx,
                       &status.legTx);
    if (fanet.isNewMsg()) {
      // write msg to udp !!
    ...
    if (fanet.getNameData(&nameData)) {
      if (setting.OGNLiveTracking.bits.fwdName) {
        ogn.sendNameData(fanet.getDevId(nameData.devId), nameData.name,
                         (float)nameData.snr / 10.0);
      }
    }
      ...
      if (msgData.dstDevId == fanet._myData.devId) {
        ...
        if (pos >= 0) {
          if (atoi(sRet.c_str()) == setting.fanetpin) {
            fanetDstId = msgData.srcDevId;  // we have to store the sender, to
                                            // send back the value !!
            log_i("got fanet-cmd from %s:%s", fanet.getDevId(fanetDstId),
                  msgData.msg.c_str());
    ...
    if (fanet.getWeatherData(&weatherData)) {
      if (setting.OGNLiveTracking.bits.fwdWeather) {
        ...
        if ((wData.wHeading >= 0) && (wData.wHeading <= 360) && (wData.bWind) &&
            (wData.wSpeed >= 0) && (wData.wSpeed <= 300) &&
            (wData.wGust >= 0) && (wData.wGust <= 300)) {
          ogn.sendWeatherData(&wData);
        }
      }
    }
    if (fanet.getTrackingData(&tFanetData)) {
      ...
      if (tFanetData.type == 0x11) {  // online-tracking
        if (setting.OGNLiveTracking.bits.liveTracking) {
          ogn.sendTrackingData(
              tFanetData.timestamp, tFanetData.lat, tFanetData.lon,
              tFanetData.altitude, tFanetData.speed, tFanetData.heading,
              tFanetData.climb, fanet.getDevId(tFanetData.devId),
              (Ogn::aircraft_t)fanet.getFlarmAircraftType(&tFanetData),
              tFanetData.addressType, tFanetData.OnlineTracking,
              (float)tFanetData.snr);
        }
        sendAWTrackingdata(&tFanetData);
        sendTraccarTrackingdata(&tFanetData);
      } else if (tFanetData.type >= 0x70) {  // ground-tracking
        if (setting.OGNLiveTracking.bits.liveTracking) {
          ogn.sendGroundTrackingData(
              tFanetData.timestamp, tFanetData.lat, tFanetData.lon,
              fanet.getDevId(tFanetData.devId), tFanetData.type,
              tFanetData.addressType, (float)tFanetData.snr);
        }
      }
#ifdef SENDFLARMDIRECT
      FlarmtrackingData myFlarmData;
      ...
      char sOut[MAXSTRING];
      int pos =
          flarm.writeFlarmData(sOut, MAXSTRING, &myFlarmData, &PilotFlarmData);
      sendData2Client(sOut, pos);
      flarmCount++;
#endif
    }
...
#ifdef AIRMODULE
    if ((setting.Mode == MODE_AIR_MODULE) ||
        ((abs(setting.gs.lat) <= 0.1) &&
         (abs(setting.gs.lon) <=
          0.1))) {  // in GS-Mode we use the GPS, if in settings disabled
      if (!status.bHasAXP192) {
      ...
      if (ppsTriggered) {
        ...
        if (nmea.isValid()) {
...
#endif
          status.GPS_Fix = 1;
          if (status.GPS_NumSat <
              4) {  // we need at least 4 satellites to get accurate position !!
            status.GPS_speed = 0;
            status.GPS_course = 0;
          } else {
            status.GPS_speed = nmea.getSpeed() * 1.852 /
                               1000.;  // speed in cm/s --> we need km/h
            status.GPS_course = nmea.getCourse() / 1000.;

            // create a global variable for logger igc file name based on GPS
            // datetime
            // set today date
          ...
          // setTime(nmea.getHour(), nmea.getMinute(), nmea.getSecond(),
          ...
          setAllTime(timeinfo);  // we have to set time to GPS-Time for
                                 // decoding.
          ...
          if (!status.vario.bHasVario)
            status.ClimbRate =
                (status.GPS_alt - oldAlt) / (float(status.tGPSCycle) / 1000.0);
          ...
          if (fanet.onGround) {
            ...
            MyFanetData.speed =
                status.GPS_speed;  // speed in cm/s --> we need km/h
          ...
          if (setting.Mode == MODE_AIR_MODULE) {
            if (setting.OGNLiveTracking.bits.liveTracking) {
              ogn.setGPS(status.GPS_Lat, status.GPS_Lon, status.GPS_alt,
                         status.GPS_speed, MyFanetData.heading);
              if (fanet.onGround) {
                ogn.sendGroundTrackingData(
                    now(), status.GPS_Lat, status.GPS_Lon,
                    fanet.getDevId(tFanetData.devId), fanet.state,
                    MyFanetData.addressType, 0.0);
              } else {
                ogn.sendTrackingData(
                    now(), status.GPS_Lat, status.GPS_Lon, status.GPS_alt,
                    status.GPS_speed, MyFanetData.heading, status.ClimbRate,
                    fanet.getMyDevId(),
                    (Ogn::aircraft_t)fanet.getFlarmAircraftType(&MyFanetData),
                    MyFanetData.addressType, fanet.doOnlineTracking, 0.0);
              }
            }
            sendAWTrackingdata(&MyFanetData);
            sendTraccarTrackingdata(&MyFanetData);
          }
          fanet.setMyTrackingData(&MyFanetData, geoidalt / 1000.,
                                  ppsMillis);  // set Data on fanet
        } else {
          status.GPS_Fix = 0;
      ...
      if (ppsTriggered) {
        ...
        MyFanetData.aircraftType = fanet.getAircraftType();
        fanet.setMyTrackingData(&MyFanetData, status.GPS_geoidAlt,
                                ppsMillis);  // set Data on fanet
      }
    }
#endif
...
void powerOff() {
  ...
  */
  ...
  while (1) {
    ...
    if ((tLogger == eDeleted) && (tBaro == eDeleted) && (tEInk == eDeleted) &&
        (tWeather == eDeleted) && (tStandard == eDeleted))
      break;  // now all tasks are stopped
    log_i("logger=%d,baro=%d,eink=%d,standard=%d,weather=%d", tLogger, tBaro,
          tEInk, tStandard, tWeather);
    delay(1000);
  }
#ifdef GSM_MODULE
  ...
  if (PinExtPowerOnOff > 0) {
    if (!digitalRead(PinExtPowerOnOff)) {
      ...
      esp_sleep_enable_ext1_wakeup(
          mask, ESP_EXT1_WAKEUP_ANY_HIGH);  // wait until power is back again
    }
  }
  // Serial.end();
  esp_wifi_stop();
...
void taskEInk(void *pvParameters) {
  ...
  if (setting.displayType == EINK2_9_V2) {
    screen.begin(1, PinEink_Cs, PinEink_Dc, PinEink_Rst, PinEink_Busy,
                 PinEink_Clk, PinEink_Din);  // display-type 1
  } else {
    screen.begin(0, PinEink_Cs, PinEink_Dc, PinEink_Rst, PinEink_Busy,
                 PinEink_Clk, PinEink_Din);
  }
  while (1) {
    screen.run();
    delay(10);
...
String(status.weather.WindSpeed,1) + "\r\nWGUST=" +
String(status.weather.WindGust,1) + "\r\nTEMP=" + String(status.weather.temp,1)
+ "\r\nPRESS=" + String(status.weather.Pressure,1); if
(!modem.sendSMS("00436769440910",sStatus.c_str())){ log_i("error sending SMS");
  }
...
void taskBackGround(void *pvParameters) {
  ...
  while (1) {
    ...
    if (setting.Mode == MODE_GROUND_STATION) {
      ...
      if (setting.gs.PowerSave == GS_POWER_SAFE) {
        if (timeOver(tAct, tRuntime, 180000))
          bPowersaveOk = true;  // after 3min. esp is allowed to go to sleep, so
                                // that anybody has a chance to change settings
        // bPowersaveOk = true; //after 3min. esp is allowed to go to sleep, so
        // that anybody has a chance to change settings
        if (status.bTimeOk == true) {
          if (!bSunsetOk) {
            bSunsetOk = true;
            /*  Time is returned in minutes elapsed since midnight. If no
             * sunrises or sunsets are expected, a "-1" is returned.
             */
    ...
    if ((WiFi.status() == WL_CONNECTED) ||
        ((status.modemstatus == MODEM_CONNECTED) &&
         (setting.wifi.connect == MODE_WIFI_DISABLED))) {
    ...
    if (WiFi.status() == WL_CONNECTED) {
      if ((!ntpOk) && (timeOver(tAct, tGetTime, 5000))) {
        ...
        struct tm timeinfo;
        if (getLocalTime(&timeinfo)) {
          log_i("h=%d,min=%d,sec=%d,day=%d,month=%d,year=%d", timeinfo.tm_hour,
...
#ifdef GSM_MODULE
    } else if ((status.modemstatus == MODEM_CONNECTED) &&
               (setting.wifi.connect == MODE_WIFI_DISABLED)) {
      if ((!ntpOk) && (timeOver(tAct, tGetTime, 5000))) {
        log_i("get ntp-time");
        byte ret = -1;
        ...
        if (ret >= 0) {
          ...
          float timezone = 0;
          xSemaphoreTake(xGsmMutex, portMAX_DELAY);
          bool bret = modem.getNetworkTime(&year3, &month3, &day3, &hour3,
                                           &min3, &sec3, &timezone);
          xSemaphoreGive(xGsmMutex);
          // log_i("h=%d,min=%d,sec=%d,day=%d,month=%d,year=%d,ret=%d",hour3,min3,
          // sec3, day3,month3, year3,bret);
          if (bret) {
            // log_i("set time");
            adjustTime(0);
            struct tm timeinfo;
    ...
    if ((setting.wifi.connect == WIFI_CONNECT_ALWAYS) &&
        (WiFi.status() != WL_CONNECTED) && (status.wifiStat)) {
    ...
    if (actFreeHeap<30000){
      ...
      {
        log_w( "*****LOOP current free heap: %d, minimum ever free heap: %d
    ******", actFreeHeap, minFreeHeap); warning_time=millis()+10000;
      }
    }
    */
    if (minFreeHeap < 20000) {
      log_e(
          "*****LOOP current free heap: %d, minimum ever free heap: %d ******",
          actFreeHeap, minFreeHeap);
    ...
    if ((tAct > (setting.wifi.tWifiStop * 1000)) &&
        (setting.wifi.tWifiStop != 0) && (!WebUpdateRunning)) {
      log_i(
          "******************WEBCONFIG Setting - WIFI "
          "STOPPING*************************");
      log_i("currHeap:%d,minHeap:%d", xPortGetFreeHeapSize(),
            xPortGetMinimumEverFreeHeapSize());
      setWifi(false);
    ...
    // if ((status.BattPerc < setting.minBattPercent) && (status.vBatt >=
    // BATTPINOK)) { // if Batt-voltage is below 1V, maybe the resistor is
    // missing.
    if ((setting.gs.PowerSave == GS_POWER_BATT_LIFE) ||
        (setting.gs.PowerSave == GS_POWER_SAFE)) {
      if ((status.BattPerc <= setting.minBattPercent) &&
          (status.vBatt >= BATTPINOK)) {
        if (timeOver(tAct, tBattEmpty, 60000)) {  // min 60sek. below
          log_i("Batt empty voltage=%d.%dV", status.vBatt / 1000,
                status.vBatt % 1000);
          log_i("sleep for 30min. --> then check again batt-voltage");
          esp_sleep_enable_timer_wakeup(
              (uint64_t)BATTSLEEPTIME *
              uS_TO_S_FACTOR);  // set Timer for wakeup
          powerOff();
        }
      } else {
      ...
      if (status.bHasAXP192) {  // power off only if we have an AXP192,
                                // otherwise, we can't switch on again.
        if ((status.vBatt < battEmpty) &&
            (status.vBatt >= BATTPINOK)) {  // if Batt-voltage is below 1V,
                                            // maybe the resistor is missing.
          if (timeOver(tAct, tBattEmpty, 60000)) {  // min 60sek. below
            log_i("Batt empty voltage=%d.%dV", status.vBatt / 1000,
                  status.vBatt % 1000);
            powerOff();  // power off, when battery is empty !!
