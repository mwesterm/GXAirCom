...
void load_configFile(SettingsData* pSetting) {
  ...
  pSetting->bHasExtPowerSw =
      preferences.getUChar("EXTPWSW", 0);  // external power-switch
  pSetting->LoraPower = preferences.getUChar("LORA_POWER", 10);  //
  pSetting->RFMode =
      preferences.getUChar("RFM", 11);  // default FntRx + FntTx + LegTx
  ...
  pSetting->outputFANETNative = preferences.getUChar("OFANETNATIVE", 0);  //
  ...
  pSetting->wifi.tWifiStop =
      preferences.getUInt("Time_WIFI_Stop", 180);  // stop wifi after 3min.
  pSetting->AircraftType = preferences.getUChar("AIRCRAFTTYPE", 1);
  pSetting->UDPServerIP = preferences.getString(
      "UDP_SERVER", "192.168.4.2");  // UDP-IP-Adress to match connected device
  pSetting->UDPSendPort =
      preferences.getUInt("UDP_PORT", 10110);  // Port of udp-server
  pSetting->outputMode = preferences.getUChar(
      "OutputMode", OUTPUT_BLE);  // output-mode default ble
  ...
  pSetting->vario.nearClimbingSensitivity =
      preferences.getFloat("vNClimbSens", 0.2);
  pSetting->vario.volume =
      preferences.getUChar("VarioVolume", 127);  // full duty-cycle
...
void write_configFile(SettingsData* pSetting) {
  ...
  preferences.putUChar("OFANETNATIVE", pSetting->outputFANETNative);  //
  ...
  preferences.putString(
      "UDP_SERVER", pSetting->UDPServerIP);  // UDP-IP-Adress for sending Pakets
  ...
  // vario
  preferences.begin("fastvario", false);
  if ((pSetting->vario.accel[0] != 0) || (pSetting->vario.accel[1] != 0) ||
      (pSetting->vario.accel[2] != 0)) {
  ...
  if ((pSetting->vario.gyro[0] != 0) || (pSetting->vario.gyro[1] != 0) ||
      (pSetting->vario.gyro[2] != 0)) {
