...
String DevelopMenue =
    "<table style=\"width:100&#37;\"><tr><td style=\"width:100&#37;\"><button "
    "onClick=\"location.href='/developmenue.html'\">developer "
    "menue</button></td></tr></table><p></p><p></p>";
String IGCMenue =
    "<table style=\"width:100&#37;\"><tr><td style=\"width:100&#37;\"><button "
    "onClick=\"location.href='/igclogs.html'\">Track Logs "
    "IGC</button></td></tr></table><p></p><p></p>";
...
void onWebSocketEvent(uint8_t client_num, WStype_t type, uint8_t *payload,
  ...
  switch (type) {
    ...
    case WStype_TEXT:
      ...
      log_i("[%u] Received text with size=%d paylod: %s", client_num, length,
            payload);
      ...
      if (root.containsKey("page")) {
        ...
        if (clientPages[client_num] == 1) {  // info
          ...
          doc["VisWeather"] =
              (uint8_t)(status.vario.bHasBME | status.bWUBroadCast);
          ...
          doc["oFanetNative"] = setting.outputFANETNative;
          ...
          doc["bHasMPU"] = (uint8_t)status.vario.bHasMPU;
          doc["vSinkTh"] =
              serialized(String(setting.vario.sinkingThreshold, 2));
          doc["vClimbTh"] =
              serialized(String(setting.vario.climbingThreshold, 2));
          doc["vNClimbSens"] =
              serialized(String(setting.vario.nearClimbingSensitivity, 2));
        ...
        command.CalibAcc =
            doc["calibAcc"].as<uint8_t>();  // calibrate accelerometer
        ...
        if (root.containsKey("setView"))
          newSetting.settingsView = doc["setView"].as<uint8_t>();
        if (root.containsKey("appw"))
          newSetting.wifi.appw = doc["appw"].as<String>();
        if (root.containsKey("wificonnect"))
          newSetting.wifi.connect = doc["wificonnect"].as<uint8_t>();
        if (root.containsKey("ssid"))
          newSetting.wifi.ssid = doc["ssid"].as<String>();
        if (root.containsKey("password"))
          newSetting.wifi.password = doc["password"].as<String>();
        if (root.containsKey("board"))
          newSetting.boardType = doc["board"].as<uint8_t>();
        if (root.containsKey("disp"))
          newSetting.displayType = doc["disp"].as<uint8_t>();
        if (root.containsKey("dispRot"))
          newSetting.displayRotation = doc["dispRot"].as<uint8_t>();
        if (root.containsKey("power"))
          newSetting.LoraPower = constrain(doc["power"].as<uint8_t>(), 0, 20);
        if (root.containsKey("band"))
          newSetting.band = doc["band"].as<uint8_t>();
        if (root.containsKey("expwsw"))
          newSetting.bHasExtPowerSw = doc["expwsw"].as<uint8_t>();
        if (root.containsKey("type"))
          newSetting.AircraftType =
              (FanetLora::aircraft_t)doc["type"].as<uint8_t>();
        if (root.containsKey("PilotName"))
          newSetting.PilotName = doc["PilotName"].as<String>();
        if (root.containsKey("output"))
          newSetting.outputMode = doc["output"].as<uint8_t>();
        if (root.containsKey("oSERIAL"))
          newSetting.bOutputSerial = doc["oSERIAL"].as<uint8_t>();
        if (root.containsKey("oGPS"))
          newSetting.outputGPS = doc["oGPS"].as<uint8_t>();
        if (root.containsKey("oFlarm"))
          newSetting.outputFLARM = doc["oFlarm"].as<uint8_t>();
        if (root.containsKey("oFanet"))
          newSetting.outputFANET = doc["oFanet"].as<uint8_t>();
        if (root.containsKey("oFanetNative"))
          newSetting.outputFANETNative = doc["oFanetNative"].as<uint8_t>();
        if (root.containsKey("oVario"))
          newSetting.outputModeVario = doc["oVario"].as<uint8_t>();
        if (root.containsKey("awlive"))
          newSetting.awLiveTracking = doc["awlive"].as<uint8_t>();
        if (root.containsKey("wifioff"))
          newSetting.wifi.tWifiStop = doc["wifioff"].as<uint32_t>();
        if (root.containsKey("UDPServerIP"))
          newSetting.UDPServerIP = doc["UDPServerIP"].as<String>();
        if (root.containsKey("UDPSendPort"))
          newSetting.UDPSendPort = doc["UDPSendPort"].as<uint16_t>();
        // gs settings
        if (root.containsKey("gslat"))
          newSetting.gs.lat = doc["gslat"].as<float>();
        if (root.containsKey("gslon"))
          newSetting.gs.lon = doc["gslon"].as<float>();
        if (root.containsKey("gsalt"))
          newSetting.gs.alt = doc["gsalt"].as<float>();
        if (root.containsKey("gsGeoAlt"))
          newSetting.gs.geoidAlt = doc["gsGeoAlt"].as<float>();
        if (root.containsKey("gsScr"))
          newSetting.gs.SreenOption = doc["gsScr"].as<uint8_t>();
        if (root.containsKey("gsPs"))
          newSetting.gs.PowerSave = doc["gsPs"].as<uint8_t>();
        if (root.containsKey("MinBatPerc"))
          newSetting.minBattPercent = doc["MinBatPerc"].as<uint8_t>();
        if (root.containsKey("restartBattPerc"))
          newSetting.restartBattPercent = doc["restartBattPerc"].as<uint8_t>();

        if (root.containsKey("mode"))
          newSetting.Mode = doc["mode"].as<uint8_t>();

        if (root.containsKey("ognlive"))
          newSetting.OGNLiveTracking.mode = doc["ognlive"].as<uint8_t>();
        if (root.containsKey("traccar_live"))
          newSetting.traccarLiveTracking = doc["traccar_live"].as<uint8_t>();
        if (root.containsKey("traccarsrv"))
          newSetting.TraccarSrv = doc["traccarsrv"].as<String>();
        if (root.containsKey("RFM"))
          newSetting.RFMode = doc["RFM"].as<uint8_t>();
        if (root.containsKey("fntMode"))
          newSetting.fanetMode = doc["fntMode"].as<uint8_t>();
        if (root.containsKey("fntPin"))
          newSetting.fanetpin = doc["fntPin"].as<uint16_t>();
        // weatherdata
        if (root.containsKey("sFWD"))
          newSetting.wd.sendFanet = doc["sFWD"].as<uint8_t>();
        if (root.containsKey("wdTempOffset"))
          newSetting.wd.tempOffset = doc["wdTempOffset"].as<float>();
        if (root.containsKey("wdWDirOffset"))
          newSetting.wd.windDirOffset = doc["wdWDirOffset"].as<int16_t>();
        if (root.containsKey("WFFNT"))
          newSetting.wd.avgFactorFanet = doc["WFFNT"].as<float>();
        if (newSetting.wd.avgFactorFanet <= 0)
          newSetting.wd.avgFactorFanet = 64;  // prevent division 0
        if (root.containsKey("WIFNT"))
          newSetting.wd.FanetUploadInterval =
              doc["WIFNT"].as<uint32_t>() * 1000;
        if (newSetting.wd.WUUploadIntervall <= 10000)
          newSetting.wd.WUUploadIntervall = 10000;
        if (root.containsKey("WFWU"))
          newSetting.wd.avgFactorWU = doc["WFWU"].as<float>();
        if (newSetting.wd.avgFactorWU <= 0)
          newSetting.wd.avgFactorWU = 128;  // prevent division 0
        if (root.containsKey("WIWU"))
          newSetting.wd.WUUploadIntervall = doc["WIWU"].as<uint32_t>() * 1000;
        if (newSetting.wd.WUUploadIntervall <= 10000)
          newSetting.wd.WUUploadIntervall = 10000;
        if (root.containsKey("wdRain"))
          newSetting.wd.RainSensor = doc["wdRain"].as<uint8_t>();
        // vario
        if (root.containsKey("vSinkTh"))
          newSetting.vario.sinkingThreshold = doc["vSinkTh"].as<float>();
        if (root.containsKey("vClimbTh"))
          newSetting.vario.climbingThreshold = doc["vClimbTh"].as<float>();
        if (root.containsKey("vNClimbSens"))
          newSetting.vario.nearClimbingSensitivity =
              doc["vNClimbSens"].as<float>();
        if (root.containsKey("vVol"))
          newSetting.vario.volume = doc["vVol"].as<uint8_t>();
        if (root.containsKey("vBeepFly"))
          newSetting.vario.BeepOnlyWhenFlying = doc["vBeepFly"].as<uint8_t>();
        if (root.containsKey("useMPU"))
          newSetting.vario.useMPU = doc["useMPU"].as<uint8_t>();
        if (root.containsKey("vTOffs"))
          newSetting.vario.tempOffset = doc["vTOffs"].as<float>();
        if (root.containsKey("axOffset"))
          newSetting.vario.accel[0] = doc["axOffset"].as<int16_t>();
        if (root.containsKey("ayOffset"))
          newSetting.vario.accel[1] = doc["ayOffset"].as<int16_t>();
        if (root.containsKey("azOffset"))
          newSetting.vario.accel[2] = doc["azOffset"].as<int16_t>();
        if (root.containsKey("gxOffset"))
          newSetting.vario.gyro[0] = doc["gxOffset"].as<int16_t>();
        if (root.containsKey("gyOffset"))
          newSetting.vario.gyro[1] = doc["gyOffset"].as<int16_t>();
        if (root.containsKey("gzOffset"))
          newSetting.vario.gyro[2] = doc["gzOffset"].as<int16_t>();
        if (root.containsKey("t[0]"))
          newSetting.vario.tValues[0] = doc["t[0]"].as<float>();
        if (root.containsKey("t[1]"))
          newSetting.vario.tValues[1] = doc["t[1]"].as<float>();
        if (root.containsKey("z[0]"))
          newSetting.vario.zValues[0] = doc["z[0]"].as<float>();
        if (root.containsKey("z[1]"))
          newSetting.vario.zValues[1] = doc["z[1]"].as<float>();
        if (root.containsKey("sigmaA"))
          newSetting.vario.sigmaA = doc["sigmaA"].as<float>();
        if (root.containsKey("sigmaP"))
          newSetting.vario.sigmaP = doc["sigmaP"].as<float>();

        // weather-underground upload
        if (root.containsKey("WUUlEnable"))
          newSetting.WUUpload.enable = doc["WUUlEnable"].as<bool>();
        if (root.containsKey("WUUlID"))
          newSetting.WUUpload.ID = doc["WUUlID"].as<String>();
        if (root.containsKey("WUUlKEY"))
          newSetting.WUUpload.KEY = doc["WUUlKEY"].as<String>();
        // Windy upload
        if (root.containsKey("WIUlEnable"))
          newSetting.WindyUpload.enable = doc["WIUlEnable"].as<bool>();
        if (root.containsKey("WIUlID"))
          newSetting.WindyUpload.ID = doc["WIUlID"].as<String>();
        if (root.containsKey("WIUlKEY"))
          newSetting.WindyUpload.KEY = doc["WIUlKEY"].as<String>();
        // GSM
        if (root.containsKey("GSMAPN"))
          newSetting.gsm.apn = doc["GSMAPN"].as<String>();
        if (root.containsKey("GSMUSER"))
          newSetting.gsm.user = doc["GSMUSER"].as<String>();
        if (root.containsKey("GSMPWD"))
          newSetting.gsm.pwd = doc["GSMPWD"].as<String>();
        if (root.containsKey("GSMMODE"))
          newSetting.gsm.NetworkMode = doc["GSMMODE"].as<uint8_t>();
        // fuel-sensor
        if (root.containsKey("fuelSensor"))
          newSetting.bHasFuelSensor = doc["fuelSensor"].as<uint8_t>();

        if (root.containsKey("MqttMode"))
          newSetting.mqtt.mode.mode = doc["MqttMode"].as<uint8_t>();
        if (root.containsKey("MqttServer"))
          newSetting.mqtt.server = doc["MqttServer"].as<String>();
        if (root.containsKey("MqttPort"))
          newSetting.mqtt.port = doc["MqttPort"].as<uint16_t>();
        if (root.containsKey("MqttPw"))
          newSetting.mqtt.pw = doc["MqttPw"].as<String>();
        ...
        if (value == 1) {
          ...
          fanetTrackingData.aircraftType =
              (FanetLora::aircraft_t)doc["type"].as<uint8_t>();
          ...
          fanetReceiver =
              strtol(doc["receiver"].as<String>().c_str(), NULL, 16);
          sendFanetData = value;
        } else if (value == 4) {
...
String processor(const String &var) {
  ...
  if (var == "SOCKETIP") {
    ...
    sRet +=
        "<table><thead><tr><th>Download</th><th>Delete</th></tr></"
        "thead><tbody>";
    while (d != NULL) {
      // Serial.println (d);
      if ((!String(d).startsWith(
              "/._"))) {  //&& !String(d).startsWith("/test"))){
    ...
    for (int i = 0; i < MAXNEIGHBOURS; i++) {
      if (fanet.neighbours[i].devId) {
        ...
        sRet +=
            " [" + fanet.getDevId(fanet.neighbours[i].devId) + "]</option>\r\n";
        // sRet += "<option value=\"" +
        // fanet.getDevId(fanet.neighbours[i].devId) + "\">" +
        // fanet.neighbours[i].name + " " +
        // fanet.getDevId(fanet.neighbours[i].devId) + "</option>\r\n";
      }
    }
    for (int i = 0; i < MAXWEATHERDATAS; i++) {
      if (fanet.weatherDatas[i].devId) {
        sRet += "<option value=\"" +
                fanet.getDevId(fanet.weatherDatas[i].devId) + "\">";
        if (fanet.weatherDatas[i].name.length() > 0) {
          sRet += fanet.weatherDatas[i].name;
        }
        sRet += " [" + fanet.getDevId(fanet.weatherDatas[i].devId) +
                "]</option>\r\n";
      }
    }
    return sRet;
    ...
    sRet = "";
    sRet =
        "<tr><th>ID</th><td>type</td><td>lat</td><td>lon</td><td>state</"
        "td><td>dist</td><td>alt</td><td>speed</td><td>climb</td><td>heading</"
        "td><td>rssi</td><td>last seen</td><td>received by</td></tr>\r\n";
    for (int i = 0; i < MAXNEIGHBOURS; i++) {
      if (fanet.neighbours[i].devId) {
        sRet +=
            "<tr><th><a "
            "href=\"https://www.google.com/maps/search/?api=1&query=" +
            String(fanet.neighbours[i].lat, 6) + "," +
            String(fanet.neighbours[i].lon, 6) + "\"  target=\"_blank\">";
        if (fanet.neighbours[i].name.length() > 0) {
          sRet += fanet.neighbours[i].name;
        }
        sRet +=
            " [" + fanet.getDevId(fanet.neighbours[i].devId) + "]</a></th>" +
            "<td>" + fanet.getAircraftType(fanet.neighbours[i].aircraftType) +
            "</td>" + "<td>" + String(fanet.neighbours[i].lat, 6) + "</td>" +
            "<td>" + String(fanet.neighbours[i].lon, 6) + "</td>" + "<td>" +
            fanet.getType(fanet.neighbours[i].type) + "</td>";
        if ((status.GPS_Lat != 0) && (status.GPS_Lon != 0)) {
          sRet += "<td>" +
                  String(distance(status.GPS_Lat, status.GPS_Lon,
                                  fanet.neighbours[i].lat,
                                  fanet.neighbours[i].lon, 'K'),
                         1) +
                  "km</td>";
        } else {
          sRet += "<td></td>";
        ...
        sRet =
            sRet + "<td>" + String(fanet.neighbours[i].altitude, 0) + "m</td>" +
            ...
            "<td>" + String(fanet.neighbours[i].rssi) + "dB</td>" + "<td>" +
            String((millis() - fanet.neighbours[i].tLastMsg) / 1000) + "s</td>";
    ...
    sRet =
        "<tr><th>ID</th><td>lat</td><td>lon</td><td>dist</td><td>temperature</"
        "td><td>wind direction</td><td>wind speed</td><td>wind "
        "gust</td><td>Humidity</td><td>barometric pressure</td><td>state of "
        "charge</td><td>rssi</td><td>last seen</td></tr>\r\n";
    for (int i = 0; i < MAXWEATHERDATAS; i++) {
      if (fanet.weatherDatas[i].devId) {
        sRet +=
            "<tr><th><a "
            "href=\"https://www.google.com/maps/search/?api=1&query=" +
            String(fanet.weatherDatas[i].lat, 6) + "," +
            String(fanet.weatherDatas[i].lon, 6) + "\"  target=\"_blank\">";
        if (fanet.weatherDatas[i].name.length() > 0) {
          sRet += fanet.weatherDatas[i].name;
        }
        sRet += " [" + fanet.getDevId(fanet.weatherDatas[i].devId) +
                "]</a></th>" + "<td>" + String(fanet.weatherDatas[i].lat, 6) +
                "</td>" + "<td>" + String(fanet.weatherDatas[i].lon, 6) +
                "</td>";
        if ((status.GPS_Lat != 0) && (status.GPS_Lon != 0)) {
          sRet += "<td>" +
                  String(distance(status.GPS_Lat, status.GPS_Lon,
                                  fanet.weatherDatas[i].lat,
                                  fanet.weatherDatas[i].lon, 'K'),
                         1) +
                  "km</td>";
        } else {
          sRet += "<td></td>";
        ...
        if (fanet.weatherDatas[i].bTemp == 1) {
          sRet +=
              "<td>" + String(fanet.weatherDatas[i].temp, 0) + "&deg;C</td>";
        } else {
          sRet += "<td></td>";
        }
        if (fanet.weatherDatas[i].bWind == 1) {
          sRet += "<td>" + getWDir(fanet.weatherDatas[i].wHeading) + " " +
                  String(fanet.weatherDatas[i].wHeading, 0) + "&deg;</td>" +
                  "<td>" + String(fanet.weatherDatas[i].wSpeed, 0) +
                  "km/h</td>" + "<td>" +
                  String(fanet.weatherDatas[i].wGust, 0) + "km/h</td>";
        } else {
          sRet += "<td></td><td></td><td></td>";
        }
        if (fanet.weatherDatas[i].bHumidity == 1) {
          sRet +=
              "<td>" + String(fanet.weatherDatas[i].Humidity, 0) + "&#37;</td>";
        } else {
          sRet += "<td></td>";
        }
        ...
        if (fanet.weatherDatas[i].bStateOfCharge == 1) {
          sRet +=
              "<td>" + String(fanet.weatherDatas[i].Charge, 1) + "&#37;</td>";
        } else {
          sRet += "<td></td>";
        }
        sRet += "<td>" + String(fanet.weatherDatas[i].rssi) + "dB</td>" +
                "<td>" +
                String((millis() - fanet.weatherDatas[i].tLastMsg) / 1000) +
                " sec.</td>" + "</tr>" + "\r\n";
      }
    }
    return sRet;
...
void onPageNotFound(AsyncWebServerRequest *request) {
  ...
  log_e("[%s] HTTP GET request of %s", remote_ip.toString().c_str(),
        request->url().c_str());
...
void Web_setup(void) {
  ...
  server.on("/fwupdate", HTTP_GET, [](AsyncWebServerRequest *request) {
    request->send(SPIFFS, request->url() + ".html", "text/html", false,
                  processor);
  });
  ...
  server.on("/download", HTTP_GET,
            [](AsyncWebServerRequest *request) { SD_file_download(request); });
...
#ifdef GSMODULE
      if ((setting.Mode == MODE_GROUND_STATION) &&
          (status.vario.bHasBME | status.bWUBroadCast)) {
        // weahter-data
        doc.clear();
        bSend = false;
