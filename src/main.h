...
#define BATTPERCSTART \
  10  // percent, where esp starts normal in addition to min. percentage
...
struct MqttModeBits {
  unsigned enable : 1, sendWeather : 1, b2 : 1, b3 : 1, b4 : 1, b5 : 1, b6 : 1,
      b7 : 1;
};
...
struct WifiSettings {
  ...
  uint8_t connect;  // 1 connect to wifi, 2 connect to wifi and try to stay
                    // connected
  uint32_t
      tWifiStop;  // time after wifi will be stopped to save energy 0 --> never
};

struct OgnModeBits {
  unsigned liveTracking : 1, sendWeather : 1, fwdWeather : 1, fwdName : 1,
      b4 : 1, b5 : 1, b6 : 1, b7 : 1;
};
union uOgnMode {
  OgnModeBits bits;
...
struct SettingsData {
  ...
  uint8_t
      fanetMode;  // fanet tracking-mode 0 ... switch between online-tracking
                  // and ground-tracking 1 ... always online-tracking
