...
class FanetLora : public Fapp {
 public:
  ...
  typedef struct {
    ...
    unsigned int
        altitude_lsb : 8; /* FANET+ reported alt. comes from ext. source */
  ...
  typedef struct {
    ...
    uint8_t
        type;  // tracking-type (11... online tracking 7X .... ground tracking)
    uint32_t devId;
    float lat;                // latitude
    float lon;                // longitude
  ...
  typedef struct {
    ...
    int rssi;                 // rssi
    uint8_t
        type;  // tracking-type (11... online tracking 7X .... ground tracking)s
    uint8_t addressType;
  } neighbour;
  ...
  typedef struct {
    ...
    bool bStateOfCharge;
    float
        Charge;  //+1byte lower 4 bits: 0x00 = 0%, 0x01 = 6.666%, .. 0x0F = 100%
  } weatherData;

  FanetLora();  // constructor
  bool begin(int8_t sck, int8_t miso, int8_t mosi, int8_t ss, int8_t reset,
             int8_t dio0, int8_t gpio, long frequency, uint8_t outputPower,
             uint8_t radio);
  void end(void);
  ...
  void getRxTxCount(uint16_t *pFntRx, uint16_t *pFntTx, uint16_t *pLegRx,
                    uint16_t *pLegTx);
  ...
  /* device-> NMEA */
  int writeFanetData(char *buffer, size_t size,
