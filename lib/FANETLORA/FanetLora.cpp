...
void FanetLora::end(void) { fmac.end(); }

String FanetLora::getMyDevId(void) { return getDevId(_myData.devId); }

bool FanetLora::begin(int8_t sck, int8_t miso, int8_t mosi, int8_t ss,
                      int8_t reset, int8_t dio0, int8_t gpio, long frequency,
                      uint8_t outputPower, uint8_t radio) {
  ...
  _PilotName = "";
  _myData.aircraftType =
      FanetLora::aircraft_t::paraglider;  // default Paraglider
  _myData.lat = NAN;
  _myData.lon = NAN;
  newMsg = false;
  ...
  bool bRet = fmac.begin(sck, miso, mosi, ss, reset, dio0, gpio, *fa, frequency,
                         outputPower, radio);
  _myData.devId =
      ((uint32_t)fmac.myAddr.manufacturer << 16) + (uint32_t)fmac.myAddr.id;
...
void FanetLora::setGPS(bool bHasGps) { fmac.bHasGPS = bHasGps; }
...
int FanetLora::addChecksum(char *buffer, size_t size) {
  uint8_t chk = 0;
  uint16_t tSize = 0;
  while (*buffer) {
...
void FanetLora::fanet_cmd_setGroundTrackingType(char *ch_str) {
  /* remove \r\n and any spaces */
  char *ptr = strchr(ch_str, '\r');
  if (ptr == nullptr) ptr = strchr(ch_str, '\n');
...
/* Transmit: #FNT
 * type,dest_manufacturer,dest_id,forward,ack_required,length,length*2hex[,signature]
 */
// note: all in HEX
...
int16_t FanetLora::getWeatherIndex(uint32_t devId, bool getEmptyEntry) {
  ...
  if (iRet >= 0) {
    ...
    return iOldestEntry;  // we tell the oldest entry to override !! (only if we
                          // to much traffic)
  }
}
...
int16_t FanetLora::getneighbourIndex(uint32_t devId, bool getEmptyEntry) {
  ...
  if (iRet >= 0) {
    ...
    if (iOldestEntry >= 0) {
      memset(&neighbours[iOldestEntry], 0,
             sizeof(neighbours[iOldestEntry]));  // clear slot
    }
    return iOldestEntry;  // we tell the oldest entry to override !! (only if we
                          // to much traffic)
  }
...
void FanetLora::clearNeighboursWeather(uint32_t tAct) {
  ...
  if ((tAct - tCheck) >= 5000) {  // check only every 5 seconds
    ...
    for (int i = 0; i < MAXNEIGHBOURS; i++) {
      if (neighbours[i].devId) {
        if ((tCheck - neighbours[i].tLastMsg) >=
            NEIGHBOURSLIFETIME) {  // if we get no msg in 4min --> del neighbour
          // log_i("clear slot %i devId
          // %s",i,getDevId(neighbours[i].devId).c_str());
    ...
    for (int i = 0; i < MAXWEATHERDATAS; i++) {
      if (weatherDatas[i].devId) {
        if ((tCheck - weatherDatas[i].tLastMsg) >=
            NEIGHBOURSLIFETIME) {  // if we get no msg in 4min --> del neighbour
          // log_i("clear slot %i devId
          // %s",i,getDevId(neighbours[i].devId).c_str());
...
void FanetLora::setPilotname(String name) { _PilotName = name; }
...
void FanetLora::setRFMode(uint8_t mode) { _RfMode = mode; }
...
Frame *FanetLora::get_frame() {
  ...
  /* in case of a busy channel, ensure that frames from the fifo gets also a
   * change */
...
void FanetLora::handle_acked(bool ack, MacAddr &addr) {
  // log_i("Handle ACK");
  char buf[64];
  snprintf(buf, sizeof(buf), "%s,%X,%X", ack ? "#FNR ACK" : "#FNR NACK",
...
String FanetLora::CreateFNFMSG(Frame *frm) {
  String msg;
  msg = "#FNF " + getHexFromByte(frm->src.manufacturer) + "," +
        getHexFromWord(frm->src.id) + "," +
        getHexFromByte(frm->dest == MacAddr()) + "," +
        getHexFromByte(frm->signature) + "," + getHexFromByte(frm->type) + "," +
        getHexFromByte(frm->payload_length) + ",";
  String payload = "";
  for (int i = 0; i < frm->payload_length; i++) {
...
int8_t FanetLora::getWeatherinfo(uint8_t *buffer, uint16_t length) {
  ...
  if (header & 8)
    expectedpacketLength += 2;  // Barometric pressure normailized (+2byte
  ...
  // integer values /
  int32_t lati = int32_t(buffer[index + 2]) << 16 |
                 int32_t(buffer[index + 1]) << 8 | int32_t(buffer[index]);
  index += 3;
  if (lati & 0x00800000) lati |= 0xFF000000;
  int32_t loni = int32_t(buffer[index + 2]) << 16 |
                 int32_t(buffer[index + 1]) << 8 | int32_t(buffer[index]);
  index += 3;
  if (loni & 0x00800000) loni |= 0xFF000000;
  ...
  if (header & 0x2) {  // state of charge
    ...
    lastWeatherData.Charge =
        float(charge) * 100 / 15;  // State of Charge  (+1byte lower 4 bits:
                                   // 0x00 = 0%, 0x01 = 6.666%, .. 0x0F = 100%)
  } else {
...
void FanetLora::handle_frame(Frame *frm) {
  ...
  */
  ...
  if (frm->type == 1) {
    // online-tracking
    actTrackingData.devId =
        ((uint32_t)frm->src.manufacturer << 16) + (uint32_t)frm->src.id;
    ...
    if (frm->payload_length <= 66) {
      ...
      // log_e("length of Frame type:%d to long
      // %d",frm->type,frm->payload_length);
      bFrameOk = false;
    }
  } else if (frm->type == 3) {
    if (frm->payload_length <= 66) {
      ...
      lastMsgData.srcDevId =
          ((uint32_t)frm->src.manufacturer << 16) + (uint32_t)frm->src.id;
      lastMsgData.dstDevId =
          ((uint32_t)frm->dest.manufacturer << 16) + (uint32_t)frm->dest.id;
      lastMsgData.msg = "";
...
void FanetLora::getRxTxCount(uint16_t *pFntRx, uint16_t *pFntTx,
                             uint16_t *pLegRx, uint16_t *pLegTx) {
...
bool FanetLora::createLegacy(uint8_t *buffer) {
  // if ((!autobroadcast) || (onGround)) //autobroadcast not enabled or on
  // ground
...
bool FanetLora::is_broadcast_ready(int num_neighbors) {
  ...
  /* in case of a busy channel, ensure that frames from the fifo get also a
   * change */
  if (next_tx > millis()) return false;

  /* determine if its time to send something (again) */
  const int tau_add = (num_neighbors / 10 + 1) * FANET_LORA_TYPE1OR7_TAU_MS;
  if (last_tx + tau_add > millis()) return false;
  // log_i("ready");
  return true;
}
...
void FanetLora::sendPilotName(uint32_t tAct) {
  static uint32_t tSend =
      millis() - SENDNAMEINTERVAL + 10000;  // send after 10seconds first name
...
uint8_t FanetLora::getFlarmAircraftType(trackingData *tData) {
  if (tData->aircraftType >= 0x80)
    return tData->aircraftType & 0x7F;  // is already a Flarm-Aircraft-Type
...
int8_t FanetLora::getTrackingInfo(Frame *frm) {
  ...
  uint8_t index = 0;
  // integer values /
  int32_t lati = int32_t(frm->payload[index + 2]) << 16 |
                 int32_t(frm->payload[index + 1]) << 8 |
                 int32_t(frm->payload[index]);
  index += 3;
  if (lati & 0x00800000) lati |= 0xFF000000;
  int32_t loni = int32_t(frm->payload[index + 2]) << 16 |
                 int32_t(frm->payload[index + 1]) << 8 |
                 int32_t(frm->payload[index]);
  index += 3;
  if (loni & 0x00800000) loni |= 0xFF000000;
  ...
  uint16_t Type =
      (uint16_t(frm->payload[index + 1]) << 8) + uint16_t(frm->payload[index]);
  index += 2;
  ...
  index++;
  int8_t climb2 = (climb & 0x7F) | (climb & (1 << 6))
                                       << 1;  // create 2-complement
  if (climb & 0x80) {
...
int16_t FanetLora::getNearestNeighborIndex() {
  ...
  for (int i = 0; i < MAXNEIGHBOURS; i++) {
    if (neighbours[i].devId) {
      pilotDistance = distance(_myData.lat, _myData.lon, neighbours[i].lat,
                               neighbours[i].lon, 'K') *
                      1000;
...
void FanetLora::writeMsgType1(trackingData *tData) { sendTracking(tData); }

void FanetLora::writeMsgType2(String name) { sendName(name); }
...
int FanetLora::serialize_tracking(trackingData *Data, uint8_t *&buffer) {
  ...
  if (std::abs(climb10) > 63)
    buffer[9] = ((climb10 + (climb10 >= 0 ? 2 : -2)) / 5) |
                (1 << 7);  // set scale factor
  else
...
int FanetLora::serialize_service(weatherData *wData, uint8_t *&buffer) {
  ...
  int iTemp = (int)(round(
      wData->temp * 2));  // Temperature (+1byte in 0.5 degree, 2-Complement)
  pkt->temp = iTemp & 0xFF;
  pkt->heading =
      uint8_t(round(wData->wHeading * 256.0 /
                    360.0));  // Wind (+3byte: 1byte Heading in 360/256 degree,
                              // 1byte speed and 1byte gusts in 0.2km/h (each:
                              // bit 7 scale 5x or 1x, bit 0-6))
  ...
  pkt->humidity = uint8_t(round(wData->Humidity * 10 /
                                4));  // Humidity (+1byte: in 0.4% (%rh*10/4))

  pkt->baro = int16_t(
      round((wData->Baro - 430.0) *
            10));  // Barometric pressure normailized (+2byte: in 10Pa, offset
                   // by 430hPa, unsigned little endian (hPa-430)*10)

  pkt->charge = constrain(roundf(float(wData->Charge) / 100.0 * 15.0), 0,
                          15);  // State of Charge  (+1byte lower 4 bits: 0x00 =
                                // 0%, 0x01 = 6.666%, .. 0x0F = 100%)
  return msgSize;
}
...
/*
 * $FNNGB,manufacturer(hex),id(hex),name(up to
 * 32bytes),type/status,latitude,longitude,altitude,climb,speed,heading*checksum
 * manufacturer:	1-2 chars hex
 * id:			1-4 chars hex
 * name:		string up to 32 chars
 * type/status:		while airborne: aircraft type: 0-7 (3D tracking), else:
 * status: 0-15 (2D tracking) +10 -> 10-25
...
int FanetLora::writeFanetData(char *buffer, size_t size,
                              FanetLora::neighbour *movePilotData) {
  snprintf(buffer, size, "$FNNGB,%x,%x,%s,%d,%.5f,%.5f,%f,%.01f,%.01f,%.f",
           movePilotData->devId >> 16, movePilotData->devId & 0xffff,
           movePilotData->name, movePilotData->type, movePilotData->lat,
           movePilotData->lon, movePilotData->altitude, movePilotData->climb,
           movePilotData->speed, movePilotData->heading);
...
String FanetLora::getDevId(uint32_t devId) {
  return getHexFromByte((devId >> 16) & 0xFF, true) +
         getHexFromByte((devId >> 8) & 0xFF, true) +
         getHexFromByte((devId)&0xFF, true);
}
